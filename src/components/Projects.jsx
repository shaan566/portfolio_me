    ...
    <section id="project" className="py-20 bg-black animate-fade-in-up text-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <div className="flex w-full sm:max-w-sm mb-10 sm:mb-12">
          <input
            className="h-11 sm:h-12 flex-grow:1 bg-gray-900 border border-gray-700 px-4 rounded-l-md focus:outline-none text-sm sm:text-base"
            type="text"
            value={search}
        ...
        {/* Projects Grid */}
        <div ref={sectionRef} className="grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
  {project
    .filter((item) =>
    ...
    .map((item, index) => (
      <div
        key={index}
        className={`
          group p-6 sm:p-8 bg-gray-900 rounded-2xl shadow-lg border border-gray-800
        ...
        `}
        style={{
          transitionDelay: `${Math.floor(index / 3) * 200}ms`,
        }}
      >
        {/* Title */}
